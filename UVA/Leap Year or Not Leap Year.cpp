...
int leap4,leap100,leap400,hul,bul;
int main()
{
    ...
    char n[1000000];
    while(scanf("%s",&n)==1){
         leap4=leap100=leap400=hul=bul=0;
         for(int i=0;i<strlen(n);i++){
             leap4=(leap4*10+n[i]-48)%4;
             leap100=(leap100*10+n[i]-48)%100;
             leap400=(leap400*10+n[i]-48)%400;
             hul=(hul*10+n[i]-48)%15;
             bul=(bul*10+n[i]-48)%55;
         }
         bool ok=0;
         if(cnt>0)pf("\n");
         cnt++;
        if((!leap4 && leap100) || !leap400){
            ...
            if(!hul)pf("This is huluculu festival year.\n");
            if(!bul)pf("This is bulukulu festival year.\n");
        }
        else{
            if(!hul)pf("This is huluculu festival year.\n");
            else pf("This is an ordinary year.\n");
        }
    }
    return 0;
}
