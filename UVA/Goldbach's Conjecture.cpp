...
bool p[MX];
vi prime;
void getprime(){prime.pb(2);for(int i=3;i<MX;i+=2){if(!p[i])prime.pb(i);}}
void sieve(){ll sq=sqrt(MX);for(int i=3;i<=sq;i+=2){if(!p[i]){for(int j=i*i;j<MX;j+=i+i){p[j]=1;}}}getprime();}
...
int main()
{
    ...
    while(si(n) && n!=0){
         bool ok=0;int a,b;
         for(int i=1;prime[i]<=n && i<prime.size();i++){
            int x = n-prime[i],p=-1;
