...
int query(int node,int s,int e,int l,int r,int k)
{
    ...
    if(l<=s&&e<=r){
        //for(auto x:tree[node])out(x);
        //outt(s,e);
        int x = upper_bound(ALL(tree[node]),k)-tree[node].begin();
       // cout << s<<" "<<e<<" "<<x<<endl;
        return tree[node].size()-x;
    }
    int mid=(s+e)/2;
    int p1=query(2*node+1,s,mid,l,r,k);
    int p2=query(2*node+2,mid+1,e,l,r,k);
    return p1+p2;
}
int main()
{
    ...
    while(q--){
      ...
      int ans = query(0,0,n-1,i-1,j-1,k);
      p=ans;
      pf("%d\n",ans);
