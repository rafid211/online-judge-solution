...
double check(double ad)
{
    double de = (ad*bc)/ab;
    double ae = (ad*ac)/ab;
    double sm = (ad+ae+de)/2;
    double ade_tri = sqrt(sm*((sm-ad)*(sm-ae)*(sm-de)));
    double sb = (ab+ac+bc)/2;
    double abc_tri = sqrt(sb*((sb-ab)*(sb-ac)*(sb-bc)));
    double bdec = abc_tri-ade_tri;

    double x = ade_tri/bdec;
